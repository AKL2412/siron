...
class GU
{
	...
	public function getServiceResponsable(\TRC\CoreBundle\Entity\Service $service){
		$sql = "SELECT DISTINCT p,f FROM TRCCoreBundle:Poste p JOIN p.service s JOIN p.fonction f WHERE p.active = true AND s = :service AND f.responsable = true";
	...
	public function getServiceScenario(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		...
		if($entite == 'même'){
			...
			$sql = "SELECT DISTINCT p FROM TRCCoreBundle:Poste p JOIN p.service s JOIN p.fonction f WHERE p.service is not null ";
		}
		...
		foreach ($result as $key => $value) {
			$idService = $value->getService()->getId();
			if(!array_key_exists($idService, $datas))
				$datas[$idService] = $value->getService();
	...
	public function aDejaDecideDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat,\TRC\CoreBundle\Entity\Poste $poste){
		$action = $this->em->getRepository('TRCCoreBundle:Demat\DematAction')
					->findBy(
						array(
							'demat'=>$demat,
							'poste'=>$poste
	...
	public function estMonDossierSNA(\TRC\CoreBundle\Entity\SNA\Dossier $dossier,\TRC\UserBundle\Entity\User $compte){
		...
		if($poste == $dossier->getRecouvreur())
			return true;
		return false;
	}

	public function membreDeMonService(\TRC\CoreBundle\Entity\Poste $poste){

		$datas = array();
		if($poste->getFonction()->getResponsable()){
			$sql = "SELECT DISTINCT p FROM TRCCoreBundle:Poste p JOIN p.service s WHERE s = :service";
			$query = $this->em->createQuery($sql);
			$query->setParameter('service',$poste->getService());
			$datas = $query->getResult();
		}
		return $datas;
	}
	public function queryPourChercherToutesMesDemat(
		\TRC\CoreBundle\Entity\Poste $poste,$admin = null){

		$criteres = array();

		if($poste->getFonction()->getResponsable()){

			$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Demat\Demat d JOIN d.auteur a JOIN  d.actions f WHERE a IN (:postes) OR f.poste IN (:postes)";

        	$criteres = array(
        		'postes'=>$this->membreDeMonService($poste)
        		);
		}else{
			

			$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Demat\Demat d JOIN d.auteur a JOIN  d.actions f WHERE a = :poste OR f.poste = :poste";
		...
		if(!is_null($admin) && $admin){
			$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Demat\Demat d WHERE d.active = :active";

			$criteres = array('active'=>true);
		}
		$sql .= " ORDER BY d.at DESC";
		$query = $this->em->createQuery($sql);
        $query->setParameters($criteres);
		return $query;
	}
	...
	public function queryPourChercherDemat(\TRC\CoreBundle\Entity\Objet\Objet $objet,
		\TRC\CoreBundle\Entity\Poste $poste){

		$criteres = array();

		if($poste->getFonction()->getResponsable()){

			$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Demat\Demat d JOIN d.instance i JOIN i.objet o JOIN d.service s JOIN d.fonctions fonc  WHERE d.statuter = false and o = :objet AND o.active = :active AND i.active = :active AND ( fonc.fonction = :fonction or d.executeur = :executeur or s = :service)";

        	$criteres = array(
        		'active'=>true,
        		'objet'=>$objet,
        		'fonction'=>$poste->getFonction(),
        		'executeur'=>$poste,
			...
			$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Demat\Demat d JOIN d.instance i JOIN i.objet o JOIN d.service s JOIN d.fonctions fonc  WHERE d.statuter = false and o = :objet AND o.active = :active AND i.active = :active AND ( (fonc.fonction = :fonction AND d.executeur is null) OR( d.executeur = :executeur) )";
		...
		//*/
		if($poste->getEmploye()->getParam()->getToutesmesdemat()){
			//$sql .= " AND p = :executeur ";
			/*

		JOIN d.actions act JOIN act.poste p
			//*/
		}
		$query = $this->em->createQuery($sql);
        $query->setParameters($criteres);
	...
	private function supprimeFonctionDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		$temps = $this->em->getRepository('TRCCoreBundle:Demat\DematFonction')
				->findByDemat($demat);
	...
	public function estOkDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		
		$documents = $this->getDocumentsDemat($demat);
		foreach ($documents as $key => $doc) {
			if(
				!is_null($doc->getDocument()) &&
				$doc->getDocument()->getObligatoire() &&
				is_null($doc->getFichier())
				)
				return false;
		}

		$documents = $this->getParametresDemat($demat);
		foreach ($documents as $key => $doc) {
			if(
				$doc->getParametre()->getObligatoire() &&
				is_null($doc->getValeur())
				)
				return false;
		}
	...
	public function uploadDematDocument($fichier,$demat,$nomdoc,$dossier){

		try {
		$message = "Rien pour le moment";
		$code = 1;
		$chemin = $demat->getDossier()."/";
		$no = $fichier['name'];
		$ext="";
		...
		$f->setNom($no);
		$f->setPath(Core::slugify($nomdoc).'-'.date('dmYHis').$ext);
		
		$doss = $chemin.$dossier."/";

		$f->setPath($doss.$f->getPath());
		...
		if(!is_dir($doss))
			...
			if(is_dir($doss) && is_writable($doss)){
				...
				if(move_uploaded_file($fichier['tmp_name'], $f->getPath())){
					$message = "ok";
				}else{
	...
	public function uploadDocument($fichier,$path,$nomdoc,$dossier){
		...
		if(!is_dir($doss))
			...
			if(is_dir($doss) && is_writable($doss)){
				...
				$message = "Erreur de téléchargement du fichier ::  // ".$e->getMessage();
			    $code = -1;
			    $f = null;
		}
		return  array(
	...
	public function removeFichier(\TRC\CoreBundle\Entity\Fichier $fichier = null){

		try {
			if(!is_null($fichier)){
				if(is_file($fichier->getPath())){

					if(unlink($fichier->getPath())){
						return array("code"=>1,
                        ...
                        'description'=>"supprimer le fichier : <b>".$fichier->getPath()."</b> Nom origine :<b>".$fichier->setNom()."</b>"
                        ));
					}
	...
	public function initierDematDocumentationParametre(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		
		$documents = $this->getDocumentationInstance($demat->getInstance());

		foreach ($documents as $key => $doc) {
			$dd = $this->em->getRepository('TRCCoreBundle:Demat\Document')
					->findOneBy(
						array(
			...
			if(is_null($dd)){
				$dd = new \TRC\CoreBundle\Entity\Demat\Document();
				$dd->setNom($doc->getNom());
				$dd->setDocument($doc);
				$dd->setDocumentation($demat->getDocumentation());
				$this->em->persist($dd);
			}else
				$dd->setActive(true);
		...
		//*
		$parametres = $this->getParametreInstance($demat->getInstance());

		foreach ($parametres as $key => $doc) {
			$dd = $this->em->getRepository('TRCCoreBundle:Demat\DematParametreValeur')
					->findOneBy(
						array(
							'conteneur'=>$demat->getParametre(),
							"parametre"=>$doc
							),
						array(),
						null,0
			...
			if(is_null($dd)){
				$dd = new \TRC\CoreBundle\Entity\Demat\DematParametreValeur();
				
				$dd->setParametre($doc);
				$dd->setConteneur($demat->getParametre());
				$this->em->persist($dd);
			}
	...
	public function getDocumentsDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Demat\Document d JOIN d.documentation doc WHERE  doc = :documentation ";
		$query = $this->em->createQuery($sql);
		$query->setParameters(
			array(
				"documentation"=> $demat->getDocumentation()
	...
	public function getParametresDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		
		$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Demat\DematParametreValeur d JOIN d.conteneur doc WHERE  doc = :conteneur ";
		$query = $this->em->createQuery($sql);
		$query->setParameters(
			array(
				"conteneur"=> $demat->getParametre()
	...
	public function getActionsDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		
		$sql = "SELECT DISTINCT m FROM TRCCoreBundle:Demat\DematAction m JOIN m.demat e WHERE e = :demat ORDER BY m.at DESC";
        $query = $this->em->createQuery($sql);
        $query->setParameter('demat',$demat);
		return $query->getResult();
	}

	public function getCommentsDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat){
	...
	public function getTracesDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat){
		
		$sql = "SELECT DISTINCT m FROM TRCCoreBundle:Demat\DematTrace m JOIN m.demat e WHERE e = :demat ORDER BY m.at DESC";
        $query = $this->em->createQuery($sql);
        $query->setParameter('demat',$demat);
        //$mes = $query->getResult();
		return $query->getResult();
	...
	private function getDocumentationInstance(\TRC\CoreBundle\Entity\Objet\Instance $instance){
		$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Objet\Document d JOIN d.documentation doc WHERE d.active = :active AND doc IN (:documentation) ";
		$query = $this->em->createQuery($sql);
		$query->setParameters(
			array(
				"active"=>true,
				"documentation"=> $instance->mesConteneursDocumentation()
				));
		return $query->getResult();
	}
	public function getParametreInstance(\TRC\CoreBundle\Entity\Objet\Instance $instance){
		$sql = "SELECT DISTINCT d FROM TRCCoreBundle:Objet\Parametre d JOIN d.conteneur doc WHERE d.active = :active AND doc IN (:conteneur) ";
		$query = $this->em->createQuery($sql);
		$query->setParameters(
			array(
				"active"=>true,
				"conteneur"=> $instance->mesConteneursParametres()
				));
		return $query->getResult();
	}
	public function definitionFonctionDemat(\TRC\CoreBundle\Entity\Demat\Demat $demat, $fonctions){
		$this->supprimeFonctionDemat($demat);
		foreach ($fonctions as $key => $f) {
			$dematfonc = new DematFonction();
	        $dematfonc->setFonction($f);
	        $dematfonc->setDemat($demat);
	        $this->em->persist($dematfonc);
		}
		$this->em->flush();
		return true;
	}
	public function detailsUtilisateur(\TRC\CoreBundle\Entity\Utilisateur $uti){
		$details = '';
		$poste = $this->getMonPoste($uti);
		if(!is_null($poste)){
			$details = 'Fonction : <b>'.$poste->getFonction()->getNom().'</b><br>'.
				'Entité : <b>'.$poste->getService()->getNom().'</b>';
		}else{
			$details = "<h5>Pas de détails</h5>";
		}
	...
	public function jeSuisEnLigne(\TRC\UserBundle\Entity\User $user){
		...
		$online = $this->em->getRepository('TRCCoreBundle:Enligne')
					->findOneBy(
						array(
							'user'=>$user,
							"online"=>true
	...
	public function jeSuisDisponible(\TRC\CoreBundle\Entity\Utilisateur $moi){

		$online = $this->em->getRepository('TRCCoreBundle:Indisponible')
					->findOneBy(
						array(
							'employe'=>$moi,
							"active"=>true,
	...
	public function delete($fileName){
		...
		if (strpos($tmp, '/', 0)!==false) {
		  ...
		  if ($deleteError) {
		    echo 'file delete error'.$tmp;
		  }
	}

	public function verificationCondition(array $conditions,$valeur){
	...
	private function evaluerExpression($valeur,$expression,$nonEval = null){
		
		$temp = "";
		$tab = explode(";", $expression);
		if(count($tab) == 2 ){
			$temp = $tab[0]." <= ".$valeur." && ".$valeur." <= ".$tab[1].";";
		}else{
			if(is_int($valeur)){
				$temp = $valeur." ".$expression." ;";
			}else{
				$temp = "'".$valeur."' ".$expression." ;";
			}
		}
	...
	public function getConditionByScenarioParametre(\TRC\CoreBundle\Entity\Core\Scenario $scenario,\TRC\CoreBundle\Entity\Objet\Parametre $parametre = null ){

		 $sql = "SELECT c FROM TRCCoreBundle:Core\Condition c JOIN c.scenario s WHERE c.active = :active AND s = :scenario";
                $query = $this->em->createQuery($sql);
                $query->setParameters(
                    array(
                        'scenario'=>$scenario,
        ...
        foreach ($cds as $key => $value) {
        	if($value->couvreParametre($parametre))
        		$array[]=$value;
        }
        return $array;
	}

	public function estInstructeurObjet(\TRC\CoreBundle\Entity\Objet\Objet $objet,\TRC\CoreBundle\Entity\Fonction $fonction){

		$sql = "SELECT c FROM TRCCoreBundle:Core\Scenario c JOIN c.objet o WHERE c.active = :active AND o IN (:objet)";
        $query = $this->em->createQuery($sql);
        $query->setParameters(
                    array(
                        'objet'=>$objet->moiObjet(),
                        'active'=>true
                        ));
        $cds = $query->getResult();
        
        foreach ($cds as $key => $value) {
        	if($value->estInstructeurScenario($fonction))
        		return true;
        }
        return false;
	}

	public function getInstructionScenario(\TRC\CoreBundle\Entity\Objet\Objet $objet,\TRC\CoreBundle\Entity\Poste $poste){
        ...
        $data = array();
        foreach ($cds as $key => $value) {
        	$scenario = $value->getInstructeurScenario($poste->getFonction());
        	if(!is_null($scenario))
        		$data[] = $scenario;
        }

        if(count($data) > 1 )
        	throw new \Exception("Chevauchement de scénarios. Veuillez contacter l'administrateur", 1);
	...
	public function getParametresInstance(\TRC\CoreBundle\Entity\Objet\Instance $instance  ){

		 $sql = "SELECT p FROM TRCCoreBundle:Objet\Parametre p JOIN c.conteneur c WHERE c.active = :active AND c IN (:conteneur)";
                $query = $this->em->createQuery($sql);
                $query->setParameters(
                    array(
                        'conteneur'=>$instance->mesConteneursParametres(),
	...
	public function getSociete($code){
		return $this->em->getRepository('TRCCoreBundle:Societe')
					->findOneByCode($code);
	}
	public function createSociete(\TRC\CoreBundle\Entity\Societe $societe){

		if(!is_null($this->getSociete($societe->getCode())))
			throw new \Exception("Erreur : Le code ".$societe->getCode()." est déjà utilisé" , 1);
		$dossier = $this->cheminPrincipal.$societe->getCode();
		$societe->setDossier($dossier);
		...
		$file = new File($societe->getImage());
			if($file != null){
                    $extension = $file->guessExtension();
                    if (!$extension) {
                        $extension = 'jpg';
                    }
                    $nomImage = $societe->getCode().' '.date('dmYHi').'.'.$extension;
                    if($file == 'ga/scte/default.jpg'){
                    	copy('ga/scte/default.jpg', $dossierImage.'/'.$nomImage);
                    }else{
                    	$file->move($dossierImage, $nomImage);
                    }
                    
                    $societe->setImage($dossierImage.'/'.$nomImage);
                }
        return $societe;
	...
	public function creerCompte($utilisateur,$roles){
		
		
		$motdepasse = "a123*123";
		try {
			$compte = new Compte();
			$compte->setIntitule($utilisateur->getPrenom()." ".strtoupper($utilisateur->getNom()));
			$compte->setImage($utilisateur->getImage());
			$compte->setEmail($utilisateur->getEmail());
			$compte->setUsername($utilisateur->getCode());
